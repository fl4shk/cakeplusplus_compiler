...
std::vector<char*> pool_for_cstm_strdup;
std::vector<int*> pool_for_cstm_intdup;
...
const char* cstm_strdup(char* some_c_str)
{
	...
	const std::string temp_str = some_c_str;
	pool_for_cstm_strdup.push_back(new char[temp_str.size() + 1]);

	{
	size_t i;
	for (i=0; i<temp_str.size(); ++i)
	{
		pool_for_cstm_strdup.back()[i] = some_c_str[i];
	}
	pool_for_cstm_strdup.back()[i] = '\0';
	}
	return pool_for_cstm_strdup.back();
}
...
const int* cstm_intdup(int num)
{
	...
	pool_for_cstm_intdup.push_back(new int());
	*pool_for_cstm_intdup.back() = num;
	return pool_for_cstm_intdup.back();
}
}
