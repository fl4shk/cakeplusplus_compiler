...
#include "backends/vm_backend_class.hpp"
...
protected:		// non-visitor functions
	...
	inline size_t calc_var_non_size_used_space(s64 type_size, size_t dim) 
	{
		if (type_size <= 0)
		{
			err(nullptr, sconcat("Frontend",
				"::calc_var_non_size_used_space():  Eek!"));
		}
	...
	inline Symbol* find_sym_or_err(antlr4::ParserRuleContext* ctx, 
		Ident ident, const std::string& error_msg)
	{
